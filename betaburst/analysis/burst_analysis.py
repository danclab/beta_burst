...
class BurstSpace:
    ...
    def __init__(self,tmin:float, tmax:float, time_step:float, perc=0.5, nb_quartiles=5):
       ...
       assert (tmax - tmin) / 2 > time_step, "`time_step` should divide total time at least in 2."
       self.perc = perc
    ...
    def fit_transform(self, burst_dict, n_components=20):
        ...
        pca_subset = np.random.choice(np.arange(amp_max.shape[0])[amp_map], size=int(amp_max.shape[0]*self.perc), replace=False)
    ...
    def waveforms_rate(self):
        ...
        self.modulation_index = np.empty((self.components.shape[1], self.nb_quartiles, len(self.binning)-1))
        self.comp_waveforms =  np.empty((self.components.shape[1], self.nb_quartiles, np.array(self.burst_dict["waveform"]).shape[-1]))
        ...
        for idx_compo in range(self.components.shape[1]): # Iterate over PC
            ...
            quartiles = np.linspace(np.min(current_compo), np.max(current_compo), self.nb_quartiles + 1)
            ...
            for idx_group in range(self.nb_quartiles):
                ...
                if idx_group != self.nb_quartiles - 1:
                    waveform_ids = np.where(
                        (current_compo >= scores_lims[0]) & (current_compo < scores_lims[1])
                    )[0]
                else:
                    waveform_ids = np.where(current_compo >= scores_lims[0])[0]

                waveform = np.mean(np.array(self.burst_dict["waveform"])[waveform_ids], axis=0) # Mean waveform corresponding to the quartile
                selected_peak_time = np.array(self.burst_dict["peak_time"])[waveform_ids] # Peaks times for the corresponding quartile
    ...
    def plot_burst_rates(self) -> None:
        ...
        x_axis = self.binning[:-1]  # Exclude the last element to match modulation_index shape
        y_axis = np.arange(self.modulation_index.shape[1])  # Index range for comps_groups
        ...
        for i in range(self.modulation_index.shape[0]):  # Iterate over the first dimension of modulation_index
            ...
            for j in range(self.nb_quartiles):
                ...
                ax_waveform.plot(self.comp_waveforms[i, j, :] + j * waveform_offset, color=colors[j], label=f'Group {j+1}')
            ...
            heatmap = ax_heatmap.imshow(self.modulation_index[i, :, :], aspect='auto',
            ...
            ax_heatmap.set_yticklabels(range(1, self.nb_quartiles + 1))  # Match group labels
    ...
    def plot_waveforms(self) -> None:
        ...
        for pc_ix, pc in enumerate(pc_labels):
            ...
            for q_ix, (b,e) in enumerate(quartiles):
                ax[pc_ix].plot(waveform_times, self.scores_dists[q_ix, pc_ix], lw=2, c=col_range[q_ix], label="Q {}".format(q_ix+1))
