...
def test_burst_extraction(bm):
    ...
    assert len(bursts[1]) > 0, "There should be noisy bursts detected on channel 1, even if we haven't added some."
    assert np.isclose(bursts[0]['peak_time'][bigger[0]], 1.081, atol = 1e-1), 'Burst not detected at the proper time locations.'
    assert np.isclose(bursts[0]['peak_time'][bigger[1]], 3.082, atol = 1e-1), 'Burst not detected at the proper time locations.'
